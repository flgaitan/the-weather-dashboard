      ...
      fetch(forecastUrl)
        .then(function(response){
          ...
          .then (function (data){
            console.log(data)
            let list = data.list 
            let actualTime = list.dt
            let actualDateFormat = new Date(actualTime * 1000)
            ...
            //console.log(useDate);
            document.querySelector(`.fpannel${xtime}Summary`).innerHTML = `<h/4>${useDate}</h4>`;
            document.querySelector(`.fpannel${xtime}Temp`).textContent = "Temperature: " + list[i].main.temp + "\u00BF";
